...
function calculateDateRetour($start_date, $duration, $holidays) {
    ...
    while ($days_added < $duration) {
        ...
        if (!isWeekend($formatted_date)) {
            ...
            if ($holiday_duration) {
                $date = strtotime('+' . ($holiday_duration - 1) . ' days', $date);
            } else {
                $days_added++;
            }
        }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $new_license_duration = $_POST['license_duration'];
    ...
    if (!$user) {
        ...
        header("Location: EditForm.php?ID_Conge=$ID_Conge");
        exit;
    }
    ...
    if ($license && $license['Rasmiya'] == 1 && $license['Supp'] == 0) {
        $new_Reste_Conge = $Rest_Conge - $new_license_duration;
    ...
    if ($license && $license['Rasmiya'] == 1 && $license['Supp'] == 0) {
        $date_retour = calculateDateRetour($date_debut, $new_license_duration, $holidays);
    } else {
        $date_retour = date('Y-m-d', strtotime($stop_date . ' + ' . $new_license_duration . ' days'));
    }
    ...
    try {
        ...
        $sql = "UPDATE conge SET 
                ID_user = :nbr_Financier, 
                ID_Contrat = :license_type, 
                Duree_Conge = :license_duration, 
                Date_Conge = :date_actuel, 
                Date_Debut = :date_debut, 
                Date_Retour = :date_retour, 
                ID_Vice = :nbr_FinancierV, 
                Remarque = :note 
                WHERE ID_Conge = :ID_Conge";
        $stmt = $dbh->prepare($sql);
        $stmt->bindParam(':nbr_Financier', $nbr_Financier, PDO::PARAM_INT);
        $stmt->bindParam(':license_type', $license_type, PDO::PARAM_INT);
        $stmt->bindParam(':license_duration', $new_license_duration, PDO::PARAM_INT);
